...
class Solution(object):
    ...
    Based on description, I assume that:
        1. Each doctor was presented by id, following with a vector of features (specific specialty, area, review score etc.). 
        Features are discretized. So doctors are coming as an matrix.
            1.1 Doctor id is an integer, or a string (name) also works.
            1.2 Specialty is an integer (Radiologist=1, Dentist=2). If we need to include similarities of the specialties, we could use a tree like numberings (General Dentist=100, Orthodontist=101, etc.)
            1.3 Area is the first 5 digits of zip code.
            1.4 Score is an int
            1.5 All features are standardized to (0,1). (Or I can pre-process it.)
    ...
    So my algorithm is to sort the doctors by something like "weighted Euclidean distance".
    
    '''
    def __init__(self, doctors):
        '''
        Input: Doctor matrix. (2d list)
        '''
    ...
    def FindSimilarDoctor(self, doc_idx):
        '''
        Input: the index of the doctor, not the id. If the input is id, we can do linear search, or binary search if ordered.
        Output: the id of the doctor ordered by similarity.
        '''
